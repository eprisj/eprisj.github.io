      ...
      <main className="mx-auto flex min-h-[80vh] w-full max-w-[560px] flex-col items-center justify-center px-6 py-20 text-center">
        ...
        {form?.support && (
          /* Единственный левополосный блок на иначе полностью центрированной
             ...
             flex justify-center сжимает aside до содержимого, а не до всей
             колонки: emphasized-карточка остаётся собранной по центру, со
             своей рамкой на всех четырёх сторонах, а не растянутой лентой.
             Это первое, что видит автор после «спасибо», и единственное
             место на этой странице, где стоит задержать взгляд дольше
             короткой черты-разделителя (h-px w-16) чуть ниже. */
          <div className="mt-10 flex w-full justify-center">
            <SupportJournal lang={form.language} emphasized />
          </div>
        )}
  ...
  return shell(
    <form onSubmit={submit} noValidate>
      ...
      {/* ПОДДЕРЖКА ПРОЕКТА.
          ...
          работы. emphasized — раскрыта сразу, крупнее и в своей карточке:
          человек дошёл до конца длинной анкеты, здесь не тот случай, чтобы
          прятать способ поддержать за кликом. Обычный, свёрнутый вид того же
          компонента остаётся в конце статей и обзоров (App.tsx). */}
      {form!.support && <SupportJournal lang={form!.language} className="mt-14" emphasized />}
