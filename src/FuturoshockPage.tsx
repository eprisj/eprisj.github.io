...
function InteriorRoom({ compact = false, lightMode = 'warm', room = 'room-01' }: { compact?: boolean; lightMode?: LightMode; room?: RoomId }) {
  ...
  return (
    <div className={`relative isolate overflow-hidden bg-[#201a17] ${compact ? 'min-h-[360px]' : 'min-h-[560px]'}`}>
      <img src="/images/futuroshock-interior.png" alt="The Futuroshock interior, prepared for its first works" className={`absolute inset-0 h-full w-full object-cover transition duration-700 ${isArchive ? 'object-[78%_center]' : 'object-center'} ${lightMode === 'contrejour' ? 'scale-[1.02] saturate-[.42] contrast-[1.28] brightness-[.56]' : 'saturate-[.72] contrast-[1.04]'}`} />
      <div className={`absolute inset-0 transition duration-700 ${lightMode === 'contrejour' ? 'bg-[linear-gradient(90deg,rgba(4,7,7,.89),rgba(10,12,12,.18)_56%,rgba(4,7,7,.76)),linear-gradient(0deg,rgba(2,4,4,.72),transparent_68%)]' : 'bg-[linear-gradient(90deg,rgba(10,12,12,.76),rgba(10,12,12,.14)_53%,rgba(10,12,12,.54)),linear-gradient(0deg,rgba(10,12,12,.7),transparent_55%)]'}`} />
      ...
      <div className={`absolute ${isArchive ? 'left-[53%] top-[24%] h-[49%] w-[32%]' : 'left-[8%] top-[27%] h-[44%] w-[38%]'} border transition duration-700 ${lightMode === 'contrejour' ? 'border-[#ffe6bd]/70 bg-[#060908]/48 shadow-[inset_0_0_80px_rgba(0,0,0,.86),14px_20px_70px_rgba(0,0,0,.64)' : 'border-[#f9e7ca]/36 bg-[#291e18]/22 shadow-[inset_0_0_50px_rgba(0,0,0,.4),0_12px_50px_rgba(0,0,0,.28)'}`}>
        <span className="absolute -left-px -top-6 font-mono text-[9px] uppercase tracking-[.17em] text-[#f8d7a0]">{isArchive ? 'archive frame / 02' : 'vacant frame / 01'}</span>
        <span className="absolute bottom-3 left-3 font-mono text-[9px] uppercase tracking-[.14em] text-white/52">2D work</span>
      </div>
      <div className={`absolute ${isArchive ? 'left-[12%] top-[40%] h-[28%] w-[20%]' : 'right-[8%] top-[33%] h-[32%] w-[23%]'} border transition duration-700 ${lightMode === 'contrejour' ? 'border-[#fff0d2]/70 bg-[#060807]/42 shadow-[inset_0_0_62px_rgba(0,0,0,.82),-15px_10px_55px_rgba(255,194,115,.22)' : 'border-[#f9e7ca]/42 bg-[#36241b]/24 shadow-[inset_0_0_45px_rgba(0,0,0,.45),0_12px_50px_rgba(0,0,0,.28)'}`}>
...
export function FuturoshockPage() {
  ...
  return (
    <main className="min-h-screen bg-[#0b0e0f] text-[#f6efe5] selection:bg-[#ee5e42] selection:text-[#0b0e0f]">
      ...
      <nav aria-label="Exhibition rooms" className="border-b border-white/12 bg-[#0b0e0f]"><div className="mx-auto grid max-w-[1700px] md:grid-cols-2 lg:px-12">{(Object.keys(ROOMS) as RoomId[]).map((room) => { const item = ROOMS[room]; const active = room === activeRoom; return <button key={room} type="button" onClick={() => { setActiveRoom(room); setLightMode(item.light); }} aria-pressed={active} className={`group grid min-h-[178px] grid-cols-[4.5rem_1fr_auto] items-end gap-5 border-b border-white/12 px-5 py-7 text-left transition sm:px-8 md:border-b-0 ${room === 'room-01' ? 'md:border-r' : ''} ${active ? 'bg-[#181514]' : 'hover:bg-white/[.045]'}`}><span className="font-display text-4xl leading-none text-[#ee9f7d]">{item.number}</span><span><span className="block font-display text-[clamp(2rem,3.5vw,3.4rem)] lowercase leading-[.86] text-[#f6efe5]">{item.title}</span><span className="mt-3 block font-mono text-[9px] uppercase tracking-[.16em] text-white/48">{item.note}</span></span><ArrowUpRight size={17} className={`mb-1 transition-transform duration-300 ${active ? 'translate-x-0 translate-y-0 text-[#ee9f7d]' : 'group-hover:translate-x-1 group-hover:-translate-y-1 text-white/45'}`} /></button>; })}</div></nav>
