...
(() => {
  ...
  // Тексты общие с публичной страницей (/brandbook/data.js): что правит
  // редакция, то читатель и видит. Здесь к ним добавляется только то, что
  // публике не нужно, — план работ по коду.
  const SHARED = (window.EPRIS_BRANDBOOK && window.EPRIS_BRANDBOOK.defaults) || {};

  const DEFAULTS = Object.assign({}, SHARED, {
    deliverables: [
      { title: 'index.css', body: 'Полный набор токенов вместо семи. Тема из «Оформления» начинает управлять всем сайтом.' },
      { title: 'tailwind.config.js', body: 'Цвета и разрядка через переменные: классы и токены перестают быть двумя разными правдами.' },
    ...
    plan: [
      { title: 'Расширить :root до полного набора токенов', risk: 'низкий',
        what: 'Добавить недостающие уровни чернил и бумаги, приглушённый текст, терракоту и цвета состояний. Визуально ничего не менять — просто дать именам существовать.',
      ...
      { title: 'Прокинуть токены в tailwind.config.js', risk: 'низкий',
        what: 'Цвета переписать через переменные, чтобы класс и токен означали одно и то же.',
        effect: 'Исчезает соблазн писать сырой HEX — есть класс.' },
      { title: 'Свести бордовые к одному', risk: 'средний',
        what: 'Пять близких оттенков заменить на основной. 41 вхождение.',
      ...
      { title: 'Три уровня чернил вместо пяти', risk: 'низкий',
        what: 'Два лишних почти-чёрных свести к основному уровню.',
        effect: 'Минус два цвета, тёмные секции становятся предсказуемыми.' },
      { title: 'Три бумаги вместо восьми', risk: 'низкий',
        what: 'Пять оттенков, отличающихся на единицу-две, свести к бумаге и листу над фоном.',
        effect: 'Разница глазу незаметна, но фон наконец однороден.' },
      { title: 'Решить судьбу терракотовых', risk: 'средний',
        what: 'Либо завести отдельный токен, либо развести по бордо и цвету ошибки.',
      ...
      { title: 'Две шкалы вместо двух дюжин значений', risk: 'средний',
        what: 'Кегли — 10 шагов, разрядка — 4 значения, оформить утилитными классами.',
        effect: 'Типографика становится узнаваемой, а не «примерно как рядом».' },
      { title: 'Зафиксировать правило', risk: 'низкий',
        what: 'Никаких сырых HEX в src/ — только токены; проверять на деплое.',
  ...
  const CORE = (window.EPRIS_BRANDBOOK?.palette?.core) || [];
  const CREAM = (window.EPRIS_BRANDBOOK?.palette?.cream) || [];
  const SEMANTIC = (window.EPRIS_BRANDBOOK?.palette?.state) || [];
  const SURFACES = (window.EPRIS_BRANDBOOK?.palette?.surfaces) || [];
  ...
  const FONTS = (window.EPRIS_BRANDBOOK?.fonts) || [];
  const SCALE = (window.EPRIS_BRANDBOOK?.scale) || [];
...
if [ -n "$hits" ]; then
  ...
  // Пишем в общий редактор панели (черновик подхватывает её автосохранение)
  // и сразу ставим публикацию в очередь: раздел читают по открытой ссылке,
  // и расхождение между тем, что видит редактор, и тем, что видит читатель,
  // здесь дороже лишнего запроса.
  ...
  function adminPassword() {
    try {
      const b64 = localStorage.getItem('epris_admin_pw_saved');
      return b64 ? atob(b64) : '';
    } catch { return ''; }
  }

  function setSaveState(text, kind) {
    const el = document.getElementById('bb-save-state');
    if (!el) return;
    el.textContent = text;
    el.className = 'bb-save-state' + (kind ? ' ' + kind : '');
  }

  function schedulePublish() {
    setSaveState('Изменено…', '');
    clearTimeout(publishTimer);
    publishTimer = setTimeout(publish, 1200);
  }
  ...
  async function publish() {
    ...
    if (!pw) {
      // Пароль хранится только при входе с «Запомнить». Без него молча
      // ничего не публикуем — но и не притворяемся, что сохранили.
      setSaveState('Сохранено в черновике — нажмите «Сохранить» слева', 'warn');
      return;
    }
    if (publishing) { publishQueued = true; return; }
    publishing = true;
    setSaveState('Публикую…', '');
    try {
      ...
      setSaveState('Опубликовано ✓', 'ok');
      announce('Изменение опубликовано');
    } catch (e) {
      setSaveState('Не удалось опубликовать — правки в черновике', 'bad');
    } finally {
      publishing = false;
  ...
  function buildHTML() {
    ...
    const type = `
      <div class="bb-grid">${FONTS.map((f) => `
        <article class="bb-card bb-card-pad">
          <div class="bb-font-head"><h3>${esc(f.name)}</h3></div>
          <p class="bb-meta-line bb-mono">${esc(f.stack)}</p>
          <p class="bb-font-sample" style="font-family:${esc(f.stack)};font-size:${f.size}px;letter-spacing:${f.tracking}">${esc(f.sample)}</p>
          <p>${esc(f.role)}</p>
        </article>`).join('')}
      </div>
      <p class="bb-note"><b>Предложение:</b> Crimson Text и PT Sans почти не держат отдельной роли — цитаты можно отдать курсиву PT Serif, служебные надписи и так наследуют базовый шрифт. Минус две гарнитуры в загрузке.</p>`;
    ...
    const editBar = !canEdit ? '' : `
      <div class="bb-editbar">
        ...
        <span class="bb-editbar-hint">${editing
          ? (contentLoaded()
            ? 'Правки публикуются сами через пару секунд после того, как вы уйдёте с поля.'
            : 'Контент не загружен: нажмите «Загрузить» в левой панели, иначе правки некуда записывать.')
          : 'Правятся суждения: правила, компоненты, референсы. Палитра и цифры берутся из кода сайта.'}</span>
        <span id="bb-save-state" class="bb-save-state" role="status" aria-live="polite"></span>
        <a class="bb-public-link" href="/brandbook/" target="_blank" rel="noreferrer">Открыть публичную страницу ↗</a>
      </div>`;
  ...
  function render() {
    ...
    try {
      ...
      // Пустая вкладка без объяснения — худший исход: раздел справочный,
      // и молчание выглядит как «панель сломалась».
      root.innerHTML = '<p class="bb-fallback">Не удалось собрать бренд-бук: '
        + String(e && e.message ? e.message : e)
        + '. Обновите страницу; если повторяется — сообщите разработчику.</p>';
      console.error('brandbook render failed', e);
    }
  }
