...
function runLocalizationHealthCheck(data) {
  ...
  /* СНАЧАЛА БАЗА, ПОТОМ ЛОКАЛИ.
     ...
     Чего заготовка НЕ делает: читателям она не показывается. getContentForLanguage
     отбрасывает заготовки из базы (`liveBase`) ещё до слияния, а isEntityLive
     ловит их второй раз. Вред у неё ровно один и косвенный — из-за неё локальный
     список расходится с базой и содержит переведённую заготовку, а это два
     сигнала, по которым mergeLocalizedItems выбрасывает ВЕСЬ языковой список.
...
// ─── Interview Studio ───────────────────────────────────────────────────────
// This is intentionally a separate workspace from the article/review canvas:
// raw recordings and their own job ids are editorial assets, not CMS entries.
// Only the explicit “В черновик статьи” action crosses that boundary.
(() => {
  const root = byId('tab-transcripts');
  if (!root) return;
  const $ = (id) => byId(id);
  const els = {
    readiness: $('transcriptReadiness'), composer: $('transcriptComposer'), newBtn: $('transcriptNewBtn'), refreshBtn: document.getElementById('transcriptRefreshBtn'), closeComposerBtn: document.getElementById('transcriptCloseComposerBtn'),
  ...
  let audioObjectUrl = '';
  let pollTimer = 0;
  let pollBackoff = 4500;
  let transcriptUploadInFlight = false;
  let transcriptAutosaveTimer = 0;
  let transcriptLastSnapshot = '';
  ...
  function audioRetentionLabel(job) {
    ...
    if (remaining <= 0) return 'аудио ожидает очистки';
  ...
  function updateStart() {
    ...
    els.start.disabled = transcriptUploadInFlight || els.start.dataset.ready !== 'true' || !sourceReady || !els.consent?.checked;
  ...
  function setTranscriptSaveState(state, message) {
    ...
    const labels = {
      saved: message || 'Все правки сохранены',
      changed: message || 'Есть несохранённые правки',
      saving: message || 'Сохраняем правки…',
      error: message || 'Не удалось сохранить — попробуйте ещё раз',
    };
    els.saveState.className = `transcript-save-state is-${state}`;
    const label = els.saveState.querySelector('span:last-child');
    if (label) label.textContent = labels[state] || labels.saved;
    els.save?.toggleAttribute('data-pending', state === 'changed' || state === 'error');
    els.undo && (els.undo.disabled = !transcriptUndoSnapshot || state === 'saving');
  }
  function editorSnapshot() {
    if (!activeJob) return '';
    return JSON.stringify(editorDraft());
  }
  function editorDraft() {
    const speakers = [...els.speakersList.querySelectorAll('[data-speaker-index]')].map((field) => field.value.trim());
    const oldSpeakers = activeJob?.speakers?.length ? activeJob.speakers : speakersFromSegments(activeJob?.segments || []);
    const renamed = new Map(oldSpeakers.map((name, index) => [name, speakers[index] || name]));
    const segments = currentSegmentsFromForm().map((segment) => ({ ...segment, speaker: renamed.get(segment.speaker) || segment.speaker }));
    return { title: els.editorTitle.textContent || '', speakers: speakers.filter(Boolean), segments, reviewed: Boolean(els.reviewed?.checked) };
  }
  function applyEditorDraft(draft) {
    if (!activeJob || !draft) return;
    activeJob.title = draft.title || activeJob.title;
    activeJob.speakers = Array.isArray(draft.speakers) ? draft.speakers : activeJob.speakers;
    activeJob.segments = Array.isArray(draft.segments) ? draft.segments : activeJob.segments;
    if (typeof draft.reviewed === 'boolean') activeJob.reviewedAt = draft.reviewed ? (activeJob.reviewedAt || 'pending') : null;
  }
  function getDraftArticleAuthor() {
    const data = parseEditorJsonSafe();
    return getRegistryAuthors(data).find((author) => author.id === draftArticleAuthorId) || null;
  }
  function renderDraftArticleAuthor() {
    if (!els.articleAuthor || !els.articleAuthorSummary) return;
    const authors = getRegistryAuthors(parseEditorJsonSafe());
    if (!authors.some((author) => author.id === draftArticleAuthorId)) draftArticleAuthorId = '';
    const options = ['<option value="">Редакция EPRIS · без карточки автора</option>']
      .concat(authors.map((author) => `<option value="${escapeHtml(author.id)}" ${author.id === draftArticleAuthorId ? 'selected' : ''}>${escapeHtml(author.name || author.id)}${author.role ? ` · ${escapeHtml(author.role)}` : ''}</option>`))
      .join('');
    els.articleAuthor.innerHTML = options;
    const selected = getDraftArticleAuthor();
    els.articleAuthorSummary.innerHTML = selected
      ? `${selected.photoUrl ? `<img src="${escapeHtml(selected.photoUrl)}" alt="" referrerpolicy="no-referrer">` : `<span aria-hidden="true">${escapeHtml((selected.name || '?').trim().charAt(0).toUpperCase() || '?')}</span>`}<div><strong>${escapeHtml(selected.name || '')}</strong>${selected.role ? `<small>${escapeHtml(selected.role)}</small>` : ''}</div>`
      : '<span>Подпись будет: EPRIS Journal · Editorial Desk</span>';
  }
  function getSearchMatches() {
    const query = els.search?.value.trim().toLocaleLowerCase() || '';
    const segments = activeJob?.segments || [];
    const indices = query ? segments.reduce((matches, segment, index) => {
      if (`${segment.speaker || ''} ${segment.text || ''}`.toLocaleLowerCase().includes(query)) matches.push(index);
      return matches;
  ...
  function reviewReasons(segment) {
    ...
    if (segment?.needsReview) reasons.push('отмечен редактором');
    if (/^(speaker|спикер)$/i.test(String(segment?.speaker || '').trim())) reasons.push('не назначен участник');
    if (text && text.length < 18) reasons.push('короткий фрагмент');
    return reasons;
  }
  function getReviewIssues(job = activeJob) {
    return (job?.segments || []).reduce((issues, segment, index) => {
      const reasons = reviewReasons(segment);
      if (reasons.length) issues.push({ index, reasons });
      return issues;
    }, []);
  }
  function focusReviewIssue(index) {
    ...
    window.requestAnimationFrame(() => {
      const target = els.segments.querySelector(`[data-segment-index="${index}"]`);
  ...
  function renderReviewSummary(job = activeJob) {
    ...
    const manual = issues.filter((issue) => issue.reasons.includes('отмечен редактором')).length;
    const currentIndex = Math.max(0, issues.findIndex((issue) => issue.index === activeReviewSegmentIndex));
    const current = issues[currentIndex] || issues[0];
    const detail = manual
      ? `${manual} ${manual === 1 ? 'место отмечено' : 'места отмечены'} вручную. Остальные подсказки не блокируют выпуск.`
      : 'Есть автоматические подсказки: они не блокируют выпуск, а помогают не пропустить место при вычитке.';
    els.reviewSummary.innerHTML = `<span class="transcript-review-summary-mark">!</span><div><strong>${issues.length} ${issues.length === 1 ? 'место стоит проверить' : issues.length < 5 ? 'места стоит проверить' : 'мест стоит проверить'}</strong><span>${escapeHtml(detail)}</span></div><button class="btn btn-sm" type="button" data-transcript-next-review="${current.index}">К следующему · ${currentIndex + 1}/${issues.length}</button>`;
  }
  function updateSearchControls(search) {
    const hasQuery = Boolean(search.query);
    const hasMatches = Boolean(search.indices.length);
    if (hasMatches && !search.indices.includes(activeSearchSegmentIndex)) activeSearchSegmentIndex = search.indices[0];
    if (!hasMatches) activeSearchSegmentIndex = -1;
    els.segmentCount.textContent = hasQuery
      ? (hasMatches ? `${search.indices.indexOf(activeSearchSegmentIndex) + 1} / ${search.indices.length} совпадений` : 'Совпадений нет')
      : `${search.total} фрагментов`;
    const disabled = !hasQuery || !hasMatches || search.indices.length < 2;
    els.searchPrev.disabled = disabled;
    els.searchNext.disabled = disabled;
  }
  function moveSearchMatch(direction) {
    const search = getSearchMatches();
    if (!search.query || !search.indices.length) return;
    const current = Math.max(0, search.indices.indexOf(activeSearchSegmentIndex));
    activeSearchSegmentIndex = search.indices[(current + direction + search.indices.length) % search.indices.length];
    renderEditor();
    window.requestAnimationFrame(() => {
      const target = els.segments.querySelector(`[data-segment-index="${activeSearchSegmentIndex}"]`);
      target?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      target?.querySelector('[data-segment-text]')?.focus({ preventScroll: true });
    });
  }
  ...
  function openLinkedArticle(job = activeJob) {
  ...
  function restoreTranscriptSnapshot() {
    if (!activeJob || !transcriptUndoSnapshot) return;
    try {
      const snapshot = JSON.parse(transcriptUndoSnapshot);
      applyEditorDraft(snapshot);
      transcriptLastSnapshot = transcriptUndoSnapshot;
      transcriptUndoSnapshot = '';
      transcriptHasUnsavedChanges = false;
      window.clearTimeout(transcriptAutosaveTimer);
      renderEditor();
      setTranscriptSaveState('saved', 'Последнее изменение отменено');
      showToast('info', 'Последнее изменение отменено локально.');
    } catch { transcriptUndoSnapshot = ''; setTranscriptSaveState('saved'); }
  }
  ...
  function renderJobProgress(job, details, jobs) {
    ...
    const paused = Boolean(processing.paused);
    const stageLabel = paused ? 'На паузе' : job?.status === 'queued' ? 'Ожидает очереди' : details.title;
    return `<div class="transcript-job-process ${paused ? 'is-paused' : ''}" aria-live="polite"><div class="transcript-progress-head"><span>${paused ? '⏸ ' : ''}${escapeHtml(stageLabel)}</span><strong>${progress}%</strong></div><div class="transcript-progress" role="progressbar" aria-label="Прогресс расшифровки" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${progress}" style="--transcript-progress:${progress / 100}"><i></i></div>${dots}${sparkline}<div class="transcript-progress-facts">${facts.map((fact) => `<span>${escapeHtml(fact)}</span>`).join('')}</div><p>${escapeHtml(details.detail)}</p>${snapshots}</div>`;
  }
  function renderJobProcessMap(job) {
    const processing = job?.processing || {};
    const phase = String(processing.phase === 'failed' ? (processing.failedPhase || 'queued') : (processing.phase || job?.status || 'queued')).toLowerCase();
    const phaseOrder = { queued: 0, checking: 1, preparing: 2, transcribing: 3, saving: 4, ready: 5 };
    const reached = job?.status === 'ready' ? 5 : (phaseOrder[phase] ?? 0);
    ...
    return `<ol class="transcript-process-map" aria-label="Карта обработки записи">${steps.map((step, index) => {
      ...
      const marker = currentState === 'failed' ? '!' : currentState === 'done' ? '✓' : String(index + 1).padStart(2, '0');
      return `<li class="is-${currentState}"><span class="transcript-process-marker" aria-hidden="true">${marker}</span><span><strong>${escapeHtml(step.label)}</strong><small>${escapeHtml(step.note)}</small></span></li>`;
    }).join('')}</ol>`;
  }
  function renderJobs(jobs) {
    ...
    const inFlight = list.filter((job) => ['queued', 'processing'].includes(job.status));
    const overallBanner = inFlight.length > 1 ? (() => {
      const avg = Math.round(inFlight.reduce((sum, job) => sum + (Number(job.progress) || 0), 0) / inFlight.length);
      const processingCount = inFlight.filter((job) => job.status === 'processing').length;
      return `<div class="transcript-overall-progress"><div class="transcript-progress-head"><span>Очередь · ${inFlight.length} ${inFlight.length === 1 ? 'запись' : inFlight.length < 5 ? 'записи' : 'записей'}${processingCount ? ` · ${processingCount} в работе` : ''}</span><strong>${avg}%</strong></div><div class="transcript-progress" role="progressbar" aria-label="Общий прогресс очереди" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${avg}" style="--transcript-progress:${avg / 100}"><i></i></div></div>`;
    })() : '';
    els.jobs.innerHTML = overallBanner + filtered.map((job) => {
  ...
  async function loadJob(id, quiet = false) {
    ...
    if (activeJob?.speakers?.length) {
      activeJob.segments = (activeJob.segments || []).map((segment) => ({ ...segment, speaker: activeJob.speakers.includes(segment.speaker) ? segment.speaker : segment.speaker }));
    }
    renderEditor();
    if (!quiet) els.editor.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }
  function scheduleTranscriptRefresh(delay = 4500) {
    window.clearTimeout(pollTimer);
    pollTimer = window.setTimeout(() => refresh(true), Math.max(1200, Math.min(30000, Number(delay) || 4500)));
  }
  async function refresh(quiet = false) {
    try {
      ...
      renderFlow(list.jobs, Boolean(health?.localEngine?.ready && health?.ffmpegReady && health?.ffprobeReady));
      if (activeJob && !transcriptHasUnsavedChanges && !transcriptSaveInFlight) await loadJob(activeJob.id, true);
      const hasWork = list.jobs.some((job) => ['queued', 'processing'].includes(job.status));
      pollBackoff = 4500;
      window.clearTimeout(pollTimer);
      if (hasWork) scheduleTranscriptRefresh(pollBackoff);
      ...
      const hasWork = latestJobs.some((job) => ['queued', 'processing'].includes(job.status));
      if (hasWork) {
        pollBackoff = Math.min(30000, Math.round(pollBackoff * 1.7));
        scheduleTranscriptRefresh(pollBackoff);
      }
      if (!quiet) showToast('error', getErrorMessage(error));
    }
  }
  function currentSegmentsFromForm() {
    const next = (activeJob?.segments || []).map((segment) => ({ ...segment }));
    els.segments.querySelectorAll('[data-segment-text]').forEach((field) => { const index = Number(field.dataset.segmentText); if (next[index]) next[index].text = field.value.trim(); });
    els.segments.querySelectorAll('[data-segment-speaker]').forEach((field) => { const index = Number(field.dataset.segmentSpeaker); if (next[index]) next[index].speaker = field.value; });
  ...
  async function saveEditor({ quiet = false, auto = false } = {}) {
    ...
    const snapshotAtSave = editorSnapshot();
    let queueFollowUpSave = false;
    try {
      transcriptSaveInFlight = true;
      setTranscriptSaveState('saving');
      const draft = editorDraft();
      const payload = await api(`/${encodeURIComponent(activeJob.id)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(draft) });
      const snapshotAfterSave = editorSnapshot();
      activeJob = payload.job;
      transcriptLastSnapshot = JSON.stringify({ title: activeJob.title || '', speakers: activeJob.speakers || [], segments: activeJob.segments || [], reviewed: Boolean(activeJob.reviewedAt) });
      transcriptUndoSnapshot = '';
      const changedWhileSaving = Boolean(snapshotAfterSave && snapshotAfterSave !== snapshotAtSave);
      if (changedWhileSaving) {
        applyEditorDraft(JSON.parse(snapshotAfterSave));
        transcriptHasUnsavedChanges = true;
        queueFollowUpSave = true;
        renderEditor();
        setTranscriptSaveState('changed', 'Есть новые правки — сохраним следом');
      } else {
        transcriptHasUnsavedChanges = false;
        renderEditor();
        setTranscriptSaveState('saved');
      }
      if (!quiet) showToast('success', 'Правки к расшифровке сохранены.');
      if (!changedWhileSaving) refresh(true);
      return true;
    } catch (error) { setTranscriptSaveState('error'); if (!quiet) showToast('error', getErrorMessage(error)); return false; }
    finally {
  ...
  function setUploadProgress({ phase = 'Передача записи', percent = 0, detail = '', state = '', visible = true } = {}) {
    if (!els.uploadProgress) return;
    const safePercent = Math.max(0, Math.min(100, Math.round(Number(percent) || 0)));
    els.uploadProgress.hidden = !visible;
    els.uploadProgress.classList.toggle('is-uploading', state === 'uploading');
    els.uploadProgress.classList.toggle('is-error', state === 'error');
    if (els.uploadPhase) els.uploadPhase.textContent = phase;
    if (els.uploadPercent) els.uploadPercent.textContent = `${safePercent}%`;
    if (els.uploadBar?.parentElement) {
      els.uploadBar.parentElement.style.setProperty('--transcript-progress', String(safePercent / 100));
      els.uploadBar.parentElement.setAttribute('aria-valuenow', String(safePercent));
    }
    if (els.uploadDetail) els.uploadDetail.textContent = detail || 'Подготавливаем передачу на VPS.';
  }
  function uploadInterviewFile(file, headers, onProgress) {
    return new Promise((resolve, reject) => {
      const xhr = new XMLHttpRequest();
      xhr.open('POST', `${INTERVIEWS_API}/upload`);
      xhr.timeout = 35 * 60 * 1000;
      Object.entries(headers).forEach(([key, value]) => xhr.setRequestHeader(key, value));
      xhr.upload.onprogress = (event) => {
        if (!event.lengthComputable) { onProgress(null, 0, 0); return; }
        onProgress(Math.round((event.loaded / event.total) * 100), event.loaded, event.total);
      };
      xhr.onerror = () => reject(new Error('Передача файла прервалась. Файл остался на устройстве — проверьте сеть и повторите отправку.'));
      xhr.ontimeout = () => reject(new Error('Передача заняла слишком много времени. Проверьте сеть и повторите отправку исходного файла.'));
      xhr.onload = () => {
        let payload = {};
        try { payload = JSON.parse(xhr.responseText || '{}'); } catch {}
        if (xhr.status >= 200 && xhr.status < 300 && payload.ok) { resolve(payload); return; }
        reject(new Error(payload.error || `VPS вернул статус ${xhr.status}`));
      };
      xhr.send(file);
    });
  }
  async function startUpload() {
    ...
    try {
      ...
      const headers = authHeaders({
        'Content-Type': file.type || 'application/octet-stream', 'X-Interview-Filename': file.name,
        'X-Interview-Title': els.title.value.trim() || file.name.replace(/\.[^.]+$/, ''), 'X-Interview-Language': els.language.value || 'en',
        'X-Interview-Speakers': els.speakers.value.trim(), 'X-Interview-Retention-Days': els.retention.value,
      });
      const payload = await uploadInterviewFile(file, headers, (percent, loaded, total) => {
        const computed = Number.isFinite(percent) ? percent : 0;
        const detail = Number.isFinite(percent)
          ? `${fmtBytes(loaded)} из ${fmtBytes(total)} передано. После 100% VPS проверит аудиодорожку и поставит запись в единственную локальную очередь.`
          : 'Передаём файл. Браузер не сообщает точный процент, но запись отправляется напрямую на VPS.';
        setUploadProgress({ state: 'uploading', percent: computed, phase: 'Передаём исходный файл', detail });
      });
      setUploadProgress({ state: 'queued', percent: 100, phase: 'Файл принят VPS', detail: 'Запись сохранена и поставлена в локальную очередь. Дальнейшие этапы видны в архиве ниже.' });
      clearComposerAfterStart();
      showToast('success', 'Запись в очереди. Можно закрыть админку — VPS продолжит обработку.');
      await refresh(true);
      els.jobs?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    } catch (error) {
  ...
  async function loadAudio() {
    if (!activeJob?.hasAudio) { showToast('info', 'Аудиофайл уже удалён по сроку хранения. Текст остаётся в архиве.'); return; }
    ...
    try {
      ...
      els.audio.scrollIntoView({ behavior: 'smooth', block: 'center' });
    } catch (error) { showToast('error', getErrorMessage(error)); }
    finally { els.loadAudio.disabled = false; els.loadAudio.textContent = originalLabel; }
  }
  ...
  async function extendAudioRetentionById(id) {
    const job = latestJobs.find((item) => String(item.id) === String(id));
    if (!job?.hasAudio) { showToast('info', 'Исходное аудио уже удалено. Расшифровка остаётся в закрытом архиве.'); return; }
    if (!window.confirm('Продлить хранение аудио ещё на 30 дней?\n\nСрок прибавится к текущей дате очистки. Текст расшифровки остаётся в архиве независимо от аудио.')) return;
    try {
      const payload = await api(`/${encodeURIComponent(id)}/retention`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ days: 30 }) });
      if (activeJob?.id === id) { activeJob = payload.job || activeJob; renderEditor(); }
      const until = payload.job?.retentionAt ? fmtDate(payload.job.retentionAt) : 'позже';
      showToast('success', `Аудио будет храниться до ${until}.`);
      refresh(true);
    } catch (error) { showToast('error', getErrorMessage(error)); }
  }
  async function deleteJobById(id) {
    const job = latestJobs.find((item) => String(item.id) === String(id));
    if (!job) return;
    if (activeJob?.id === id && (transcriptHasUnsavedChanges || transcriptSaveInFlight)) {
      showToast('info', 'Сначала сохраните правки открытой расшифровки.');
      return;
    }
    const articleNote = job.articleDraft?.id ? ` Связанная статья #${job.articleDraft.id} останется без изменений.` : '';
    if (!window.confirm(`Удалить рабочую запись «${job.title || job.filename || 'Без названия'}»?\n\nБудут удалены аудио и расшифровка из закрытого архива. Это действие нельзя отменить.${articleNote}`)) return;
    try {
      ...
      if (activeJob?.id === id) {
        ...
        audioObjectUrl = '';
        activeJob = null;
        transcriptLastSnapshot = '';
        transcriptUndoSnapshot = '';
        transcriptHasUnsavedChanges = false;
        renderEditor();
      }
      showToast('success', job.articleDraft?.id ? `Рабочая запись удалена. Статья #${job.articleDraft.id} сохранена.` : 'Рабочая запись удалена.');
      refresh(true);
    } catch (error) { showToast('error', getErrorMessage(error)); }
  }
  async function createArticle() {
    ...
    try {
      ...
      const groups = [];
      for (let start = 0; start < activeJob.segments.length; start += 12) {
        const block = activeJob.segments.slice(start, start + 12).map((segment) => `<strong>${escapeHtml(segment.speaker || 'Speaker')}</strong><br>${escapeHtml(segment.text || '')}`).join('<br><br>');
        groups.push({ type: 'text', content: block });
      }
  ...
  els.start?.addEventListener('click', startUpload); els.jobs?.addEventListener('click', (event) => { const clearFilters = event.target.closest('[data-interview-clear-filters]'); if (clearFilters) { archiveFilter = 'all'; if (els.archiveSearch) els.archiveSearch.value = ''; renderJobs(latestJobs); return; } const draftButton = event.target.closest('[data-interview-open-draft]'); if (draftButton) { openLinkedArticle(latestJobs.find((job) => String(job.id) === String(draftButton.dataset.interviewOpenDraft))); return; } const retentionButton = event.target.closest('[data-interview-retention]'); if (retentionButton) { extendAudioRetentionById(retentionButton.dataset.interviewRetention); return; } const deleteButton = event.target.closest('[data-interview-delete]'); if (deleteButton) { deleteJobById(deleteButton.dataset.interviewDelete); return; } const retryButton = event.target.closest('[data-interview-retry]'); if (retryButton) { retryJobById(retryButton.dataset.interviewRetry); return; } const pauseButton = event.target.closest('[data-interview-pause]'); if (pauseButton) { api(`/${encodeURIComponent(pauseButton.dataset.interviewPause)}/pause`, { method: 'POST' }).then(() => refresh(true)).catch((error) => showToast('error', getErrorMessage(error))); return; } const resumeButton = event.target.closest('[data-interview-resume]'); if (resumeButton) { api(`/${encodeURIComponent(resumeButton.dataset.interviewResume)}/resume`, { method: 'POST' }).then(() => refresh(true)).catch((error) => showToast('error', getErrorMessage(error))); return; } const button = event.target.closest('[data-interview-open]'); if (button) loadJob(button.dataset.interviewOpen); });
  ...
  els.audio?.addEventListener('timeupdate', () => {
    ...
    const match = [...(els.segments?.querySelectorAll('[data-segment-start]') || [])].find((node) => {
      ...
      const end = Number(node.dataset.segmentEnd) || start + 30;
      return t >= start && t < Math.max(end, start + 0.1);
    });
    if (match !== lastPlayingSegmentEl) {
      lastPlayingSegmentEl?.classList.remove('is-playing');
      match?.classList.add('is-playing');
      lastPlayingSegmentEl = match || null;
    }
  });
  els.audio?.addEventListener('pause', () => { lastPlayingSegmentEl?.classList.remove('is-playing'); lastPlayingSegmentEl = null; });
  els.playerSpeed?.addEventListener('click', (event) => {
    const button = event.target.closest('[data-speed]');
    if (!button || !els.audio) return;
    els.audio.playbackRate = Number(button.dataset.speed) || 1;
    els.playerSpeed.querySelectorAll('button').forEach((b) => b.classList.toggle('is-active', b === button));
...
(function initHomepageTab() {
  ...
  function homepageArchiveCountLabel(count) {
    const last = Math.abs(Number(count) || 0) % 10;
    const lastTwo = Math.abs(Number(count) || 0) % 100;
    const word = lastTwo >= 11 && lastTwo <= 14 ? 'выпусков' : last === 1 ? 'выпуск' : last >= 2 && last <= 4 ? 'выпуска' : 'выпусков';
    return `${count} ${word}`;
  }
          ...
          <div class="homepage-archive-thumbs">
            ${visibleCards.map((card) => {
              ...
              return `<article class="homepage-archive-thumb">
                ...
                <div class="homepage-archive-thumb-copy"><span>${esc(card?.categoryLabel || card?.category || 'Фото')}</span><strong>${esc(card?.title || 'Без названия')}</strong><p>${esc(card?.description || 'Краткое описание не заполнено.')}</p></div>
              </article>`;
            }).join('')}
          </div>
...
(function () {
  ...
  // Activate panel when token is available
  function tryActivate() {
    const token = getToken();
    const statusEl = document.getElementById('podcastsTabStatus');
    const panel = document.getElementById('podcastsTabPanel');
    if (!panel) return;
    if (token) {
  ...
  function renderList() {
    ...
    if (!el) return;
    if (!_list.length) { el.innerHTML = '<p style="padding:24px;text-align:center;font-size:.8rem;color:var(--text-muted)">Нет эпизодов.</p>'; return; }
    const groups = {};
    _list.forEach(p => { const s = (p.season || '').trim() || '—'; (groups[s] = groups[s] || []).push(p); });
    el.innerHTML = Object.entries(groups).map(([season, items]) => `
        ...
        ${items.map(p => `
          <div style="display:flex;align-items:center;gap:12px;padding:10px 20px;border-bottom:1px solid var(--line)">
            ...
            <div style="display:flex;gap:5px;flex-shrink:0">
              <button class="btn btn-sm" type="button" onclick="window._ptTogglePub(${p.id})" style="color:${p.is_published ? 'var(--ok)' : 'var(--text-muted)'}">${p.is_published ? '👁' : '○'}</button>
              <button class="btn btn-sm" type="button" onclick="window._ptEdit(${p.id})">✎</button>
              <button class="btn btn-sm btn-danger-text" type="button" onclick="window._ptDelete(${p.id},'${(p.title||'').replace(/'/g,"\\'")}')">✕</button>
            </div>
          </div>`).join('')}
      </div>`).join('');
  }
  ...
  function showForm(p = null) {
    _editing = p ? p.id : null;
    const form = document.getElementById('podcastsTabForm');
    if (!form) return;
    document.getElementById('ptTitle').value = p?.title || '';
    document.getElementById('ptEpisode').value = p?.episode || '';
    document.getElementById('ptAudioUrl').value = p?.audio_url || '';
    document.getElementById('ptCoverUrl').value = p?.cover_url || '';
    document.getElementById('ptDuration').value = p?.duration || '';
    document.getElementById('ptSeason').value = p?.season || '';
    document.getElementById('ptTags').value = p?.tags || '';
    document.getElementById('ptDesc').value = p?.description || '';
    document.getElementById('ptPublished').checked = p ? !!p.is_published : true;
    document.getElementById('ptEditId').value = p?.id || '';
    form.style.display = 'block';
  }
  ...
  async function saveEpisode() {
    ...
    const body = {
      title: document.getElementById('ptTitle').value.trim(),
      audio_url: document.getElementById('ptAudioUrl').value.trim(),
      cover_url: document.getElementById('ptCoverUrl').value.trim(),
      description: document.getElementById('ptDesc').value.trim(),
      duration: document.getElementById('ptDuration').value.trim(),
      season: document.getElementById('ptSeason').value.trim(),
      episode: parseInt(document.getElementById('ptEpisode').value) || 0,
  ...
  document.getElementById('podcastsTabNewBtn')?.addEventListener('click', () => showForm());
  document.getElementById('podcastsTabRefreshBtn')?.addEventListener('click', loadList);
  document.getElementById('ptSaveBtn')?.addEventListener('click', saveEpisode);
  document.getElementById('ptCancelBtn')?.addEventListener('click', () => { document.getElementById('podcastsTabForm').style.display = 'none'; });

  // Activate when tab is opened
  document.querySelector('[data-tab="podcasts"]')?.addEventListener('click', tryActivate);
  // Also activate if token already present on load
  setTimeout(tryActivate, 600);
})();

// ══════════════════════════════════════════════════════════
// DESIGN TAB — catalog stats + VPS API health
// ══════════════════════════════════════════════════════════
(function () {
  ...
  async function testCurate() {
    const el = document.getElementById('designCurateStatus');
    if (el) el.textContent = 'Отправляю тестовый запрос…';
    try {
      const r = await fetch(DESIGN_API + '/design/curate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ brief: 'Test', items: [] }),
        signal: AbortSignal.timeout(15000),
      });
      const j = await r.json();
      if (el) { el.textContent = j.title ? `✓ AI ответил: «${j.title}»` : '✓ Эндпоинт доступен'; el.style.color = 'var(--ok)'; }
    } catch (e) {
  ...
  async function clearVpsCache() {
    const btn = document.getElementById('designClearCacheBtn');
    if (btn) { btn.disabled = true; btn.textContent = 'Очищаю…'; }
    try {
      const r = await fetch(DESIGN_API + '/design/cache/clear', { method: 'POST', signal: AbortSignal.timeout(6000) });
      const j = await r.json().catch(() => ({}));
      const statusEl = document.getElementById('designApiStatus');
      if (statusEl) { statusEl.textContent = j.ok ? '✓ Кеш VPS очищен' : (j.error || 'Готово'); statusEl.style.color = 'var(--ok)'; }
    } catch (e) {
      const statusEl = document.getElementById('designApiStatus');
      if (statusEl) { statusEl.textContent = '✗ ' + e.message; statusEl.style.color = 'var(--danger)'; }
    } finally {
      if (btn) { btn.disabled = false; btn.textContent = 'Очистить кеш VPS'; }
    }
  }

  document.getElementById('designCheckApiBtn')?.addEventListener('click', loadDesignStats);
